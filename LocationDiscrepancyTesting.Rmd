...
```{r}
start_date <- ymd(20250219)
end_date <- ymd(20250225)
...
location_trx <- location %>%
  select(id, svc_date, operator_name, stop_id, stop_name, latitude, longitude, external_stop_point_id) %>%
...
write.csv(rs_device_location, file = paste0(user_path, "/OneDrive - MBTA/Desktop/location_discrepancies_test.csv"), row.names = F)
```