...
start_date <- ymd(20250312)
end_date <- ymd(20250312)
```
...
write.csv(trx_all_append, file = paste0(user_path, "/OneDrive - MBTA/Desktop/transactions_03_12.csv"), row.names = F)
```
...
```{r}
trx_dmap <- dbGetQuery(con_dmap, paste0("SELECT transaction_dtm, external_stop_point_id FROM public.use_transaction_location WHERE transaction_dtm >= '2025-03-12'"))
```
...
trx_dmap_0312 <- trx_dmap %>%
  filter(svc_date == '2025-03-12')

sum(is.na(trx_dmap_0312$external_stop_point_id))
```
