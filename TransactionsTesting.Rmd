...
trx_cubic_recent <- dbGetQuery(con_dmap, paste0(
  ...
  device_id, fare_prod_desc AS category_name, fare_prod_rider_class_desc AS fare_prod_name, bus_id, operator_name, latitude lat, longitude lon, external_stop_point_id stop_id,
...
trx_all_append <- trx_cubic_recent_location %>%
  filter(system == "AFC2")

trx_all_append <- trx_all_append %>%
  rename(txn_time = transaction_dtm) %>%
  select(svc_date, txn_time, device_id, devicetype, category_name, fare_prod_name,
         system, operator_name, stop_compiled, stop_id) %>%
  rbind(trx_stage_location %>%
          transmute(svc_date, txn_time, device_id, devicetype, category_name, 
                    fare_prod_name, system, operator_name, stop_compiled, stop_id 
                    = odx_stop_id)) %>%
...
#write.csv(trx_all_append, file = paste0(user_path, "/OneDrive - MBTA/Desktop/transactions_02_28.csv"), row.names = F)
```
...
```{r}
table(trx_stage_location$odx_stop_id)
```
