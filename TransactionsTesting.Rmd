...
start_date <- ymd(20250315)
end_date <- ymd(20250315)
```
...
#write.csv(trx_all_append, file = paste0(user_path, "/OneDrive - MBTA/Desktop/transactions_10_15.csv"), row.names = F)
```

```{r}
temp_table <- trx_stage_location %>%
  filter(stop_id_compiled == 'Unknown')
```

```{r}
transactions_feb_new <- read.csv(paste0(user_path, "/OneDrive - MBTA/Desktop/transactions_feb_new.csv"))

transactions_feb_old <- read.csv(paste0(user_path, "/OneDrive - MBTA/Desktop/transactions_feb_old.csv"))
```

```{r}
table(trx_all_append$stop_id)
```

```{r}
sum(is.na(trx_all_append$stop_id))
```

```{r}
trx_afc1 <- trx_all_append %>%
  filter(system == "AFC1")

trx_afc2 <- trx_all_append %>%
  filter(system == "AFC2")
```

```{r}
sum(is.na(trx_afc1$stop_id))

sum(is.na(trx_afc2$stop_id))
```

```{r}
sum(is.na(trx_stage$odx_stop_id))
```

```{r}
trx_dmap <- dbGetQuery(con_dmap, paste0("SELECT transaction_dtm, 
external_stop_point_id FROM public.use_transaction_location WHERE 
transaction_dtm >= '2024-10-15'"))
```
...
trx_dmap_1015 <- trx_dmap %>%
  filter(svc_date == '2024-10-15')

sum(is.na(trx_dmap_1015$external_stop_point_id))
```
